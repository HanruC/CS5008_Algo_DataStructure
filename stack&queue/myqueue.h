...
int queue_enqueue(queue_t *q, int item)
{
    ...
    q->data[q->back] = item; // back: next available position that need to be insert. First available is 0, the second is 1. 
    q->back++;   
    if (q->back == q->capacity){
        q->back = 0;  //set to 0 which is the head when you add to the last one and reach the capacity. 
    }
    q->size++;
    return 0;
}
...
int queue_dequeue(queue_t *q)
{
    ...
    int remove_item = q->data[q->front]; // remove from head. 
    q->front++;
    if (q->front == q->capacity){
        q->front = 0; // set back to the initial position when reach the end pos
    }
    q->size--;
    return remove_item;// Note: This line is a filler so the code compiles.
}
...
{
    if (q == NULL){
        fputs("queue not created!", stderr);
        return -1;
    }
    return q->size;
}
