...
// stack logic for linked list.
// add from the tail and pop from the head. (Last in First Out).
...
int stack_enqueue(stack_t *s, int item)
{
    ...
    if (s->count == 0){
        ...
        newNode->next = s->head; // add from the bottom to up. newNode->next is the second newNode.
        //for stack, the second newNode will be added before the first one. 
        //so newNode next will be the stack's head. 
        s->head = newNode;
    }
    s->count++;
    return 0; 
...
int stack_dequeue(stack_t *s)
{
    ...
    int remove_item = s->head->data; 
    node_t* pointer = s->head;
    s->head = s->head->next;
    free(pointer);
    s->count--;
    return remove_item;
